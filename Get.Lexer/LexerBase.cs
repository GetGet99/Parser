...
namespace Get.Lexer;
/// <summary>
/// Base class for lexers generated by <c>Get.Lexer.SourceGenerator</c>.
/// Drives DFA-based regex matching over an <see cref="ITextSeekable"/> input stream,
/// producing a sequence of <see cref="IToken{TToken}"/> values.
/// </summary>
/// <typeparam name="TState">The lexer state enum type (for stateful lexing).</typeparam>
/// <typeparam name="TTokenEnum">The terminal token enum type.</typeparam>
public abstract class LexerBase<TState, TTokenEnum> where TTokenEnum : Enum where TState : Enum
{
    /// <summary>Represents a token with a typed data payload.</summary>
    ...
    /// <summary>Represents a token with no data payload.</summary>
    public record class Token(Position Start, Position End, TTokenEnum TokenType) : IToken<TTokenEnum>
    {
        public override string ToString()
            => $"{Start}-{End}: {TokenType}";
    }
    /// <summary>Returns true when the input seeker has exhausted all characters.</summary>
    ...
    /// <summary>When set to true, the lexer stops producing tokens even if more input is available.</summary>
    ...
    /// <summary>
    /// Initializes the lexer with a text source and initial state.
    /// </summary>
    /// <param name="text">The seekable text source to tokenize.</param>
    /// <param name="initialState">The initial lexer state.</param>
    ...
    /// <summary>The text matched by the current token (between the last and current DFA match positions).</summary>
    protected string MatchedText { get; private set; } = "";
    /// <summary>Raw seeker position at the start of the current match.</summary>
    protected int MatchedStartRawPosition { get; private set; }
    /// <summary>Raw seeker position at the end of the current match.</summary>
    protected int MatchedEndRawPosition { get; private set; }
    /// <summary>Zero-based start position (line, char) of the current match.</summary>
    protected Position MatchedStartPosition { get; private set; }
    /// <summary>Zero-based end position (line, char) of the current match.</summary>
    ...
    /// <summary>The current lexer state, used for stateful tokenization.</summary>
    ...
    /// <summary>
    /// Tokenizes the input source text, yielding a sequence of tokens.
    /// </summary>
    /// <remarks>
    /// Each call advances the DFA through the input. After the final token,
    /// <see cref="GetEOFToken"/> is called to optionally emit a terminal EOF token.
    /// </remarks>
